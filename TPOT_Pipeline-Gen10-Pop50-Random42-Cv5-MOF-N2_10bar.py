...
tpot_data = pd.read_csv('MOF_N2_10BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('NN2 10 Bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['NN2 10 Bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.0038673733621478667
exported_pipeline = make_pipeline(
    ...
    StackingEstimator(estimator=ExtraTreesRegressor(bootstrap=True, max_features=0.6500000000000001, min_samples_leaf=11, min_samples_split=14, n_estimators=100)),
    XGBRegressor(learning_rate=0.1, max_depth=8, min_child_weight=5, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.45, verbosity=0)
)
